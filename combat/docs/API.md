...
projectile↔projectile clashes (Layer 1) and projectile↔agent hits (Layer 2),
...
// Submit EVERY FRAME while the projectile lives. Owner's team + taiji flag
...
## 4. Projectile → Agent rules (Layer 2)
...
- **Đạn Thổ trong Rừng**: `ELEM_EARTH` projectile positioned inside a
  `NAT_FOREST` zone deals 50% damage (design doc §XI; rule mirrored in
  `game/game_rules.c`).
...
|---|---|---|---|---|
| 2026-07-20 | Claude | Doc migration: moved from root `combat/docs/API.md` to `combat/docs/API.md` per `DOC_ARCHITECTURE.md`; split "Migrated so far" progress note into `PROGRESS.md` | root `combat/docs/API.md` (read directly) | Ground-truth |
