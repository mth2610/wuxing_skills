...
- `MapProp_CreateFlowerField` builds volumetric crossed stems, leaves, bloom heads, and flower centers. With `petalTexturePath`, each head becomes one authored full-bloom alpha-cutout card plus an opaque 3D pollen center; `atlasColumns`/`atlasRows` and per-placement `bloomVariant` select different species silhouettes. Texture RGB is reduced to luminance, so runtime tint owns all flower hue. `maps/toolkit/textures/wildflower_bloom_atlas_v2.png` is the reusable neutral 4×2 atlas (eight species at 256² per cell). Without a texture, the builder falls back to procedural petal geometry. Every field also owns a compensated far mesh that keeps one tapered stem and the authored bloom silhouette, reducing textured flowers from 36 to 12 vertices without changing their distribution or tint. Each field stores a conservative bounding sphere and is rejected before shader setup when outside the view frustum or its quality-scaled draw range; use `MapProp_SetFlowerFieldDrawDistance` to tune that range and `MapProp_SetFlowerFieldLod` to tune the quality-scaled far-LOD and contact-shadow ranges. A visible field remains one vegetation draw call plus, only at useful MED/HIGH distances, one batched multiplicative contact-shadow draw. LOW/UNLIT omit that second draw. Large maps should create localized fields (typically one per authored flower patch) so invisible patches cost no draw call. Shadow direction updates from the current environment light without issuing a draw per flower.

Call `MapProp_ResetNatureRenderStats()` once before a map's nature draws, then read `MapProp_GetNatureRenderStats()` for submitted near/far meadow chunks, near/far flower fields, frustum/distance rejects, and flower/shadow draw counts. `flowerShadowDistanceCulled` isolates visible fields whose contact shadow was intentionally skipped. These counters measure CPU submission decisions; use platform GPU timing separately for shader/fill cost.
