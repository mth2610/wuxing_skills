...
- `MapProp_CreateMeadow` builds curved multi-segment foliage blades from caller-owned placements. Every blade terminates in one pointed triangle instead of a blunt final quad, saving three vertices per blade while improving its silhouette. It partitions the field into configurable chunks, builds compensated near/far geometry, and selects/culls chunks by distance plus a conservative camera-frustum sphere test. Draw and near-LOD ranges scale with `GfxQuality`, so mobile tiers submit fewer chunks without rebuilding the field. LOD thresholds are spatially staggered over a 4 m band and retain per-chunk hysteresis, preventing a visible circular switch front and camera-boundary thrashing without adding alpha discard. Set `shadowDistance` to opt into one six-vertex ground-contact wedge per clump, batched by the same chunks; only nearby visible chunks submit the multiplied shadow pass at MED/HIGH. LOW/UNLIT neither allocate nor draw these optional meshes. Untextured geometry uses a dedicated opaque shader with no alpha discard, preserving Early-Z/HSR on tile-based mobile GPUs; only authored cutout foliage uses `nature_lit.fs`. An optional second UV channel supports cutout materials while the first remains dedicated to wind phase/root masking. `MapProp_DrawMeadow` applies coherent macro gusts, restrained per-plant flutter, two-sided thin-foliage transmission, procedural root contact occlusion, environment directional shadows, and VFX point lights.
- `MapProp_BeginNatureInteraction(focus, dt)`, `MapProp_AddNatureInteractor(position, radius, strength)`, and `MapProp_EndNatureInteraction()` update one moving 64×64 vector-displacement field. Begin scrolls the local 18 m field with the focus and applies exponential recovery; call Add any number of times for players, monsters, or vehicles; End performs one 16 KB texture upload. Call `MapProp_AddNatureWindVorticles(time)` once between Begin/End to make both meadows and flower fields receive the Core Wind system. Vegetation's broad world-space grass waves now mirror `Wind_GetMacroAt`'s hash-gradient noise, drift, base velocity, and gust amplitude instead of running independent sine waves. MED/HIGH evaluate both horizontal noise channels; LOW/UNLIT retain the same advected field and use one channel along the base direction to halve the gradient-noise work. Grass and flowers transform that shared velocity through different receiver profiles: grass reacts quickly with more transverse flutter, while flowers lag, bend less, and oscillate more slowly. Flutter amplitude is proportional to live wind deflection, so zero wind produces zero procedural motion. The strongest Linear Gust or Radial Blast is sent to both vegetation vertex shaders as a direct analytic impact. A Radial Blast derives its outward direction per plant and travels as a soft expanding annulus, so nearby plants react first and the pressure front then sweeps outward; it is not rendered as a filled Perlin patch. Vortex and Turbulence remain spatially sampled and superposed in the interaction texture using the exact Core evaluator, because collapsing either field to one direction would rotate or flip the whole patch as a rigid sheet. Guided Particle presents the radial impulse briefly, then its Turbulence ramps over 0.2 s into the stronger, longer-lived primary motion. Every custom-uniform upload and its vegetation draw stay inside the same explicit `BeginShaderMode`/`EndShaderMode` scope, which is required by rlvk. Because this engine's `matModel` includes the view transform, both vertex shaders apply `u_worldFromShaderSpace` before comparing a vertex against world-space receiver centres. The receiver window follows the dominant impact, roots remain pinned by the height mask, and visible geometry plus dynamic shadow casters use the same deformation. Set `WUXING_WIND_RECEIVER_TRACE=1` to log Guided cast/arrival, selected receiver source, shader id, reflected uniform status, radius, bend strength, and normalized impact age. Call `MapProp_ClearNatureInteraction()` on map unload to release the texture and prevent old trails leaking into the next map.
...
- `MapProp_CreateWaterSurface` builds a tessellated lake plus an irregular seed-driven, multi-ring bank that slopes from the waterline to `bankGroundY`. Its shader combines multi-directional displacement, reconstructed wave normals, texture-driven microdetail, Fresnel response, sun glint, depth color, restrained broken shoreline foam, and VFX point-light response. `detailScale` and `detailStrength` tune reusable surface breakup; `MapProp_GetWaterEdgePoint` places reeds/rocks against the exact generated shoreline. Alpha is always `1.0`.
