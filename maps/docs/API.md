...
- `MapProp_CreateMeadow` builds curved multi-segment blade clumps from caller-owned placements. It partitions the field into configurable chunks, builds compensated near/far geometry, and selects/culls chunks by camera distance. `MapProp_DrawMeadow` applies coherent macro gusts, restrained per-plant flutter, sun/ambient lighting, back-lighting, and VFX point lights.
...
- `MapProp_CreateFlowerField` builds volumetric crossed stems, raised petals, and flower centers. Each placement supplies petal count, color, scale, rotation, and wind phase; the whole field remains one draw call.
