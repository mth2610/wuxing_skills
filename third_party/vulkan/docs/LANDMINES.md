...
- **A raymarched world volume is vertically displaced after an RT→screen negative-height draw, while atlas flips create mirrored or duplicate lobes.** This is not the §7.26 MODELVIEW leak (`shadow_pipeline` still passes): the shader must undo the final display flip before inverse projection, while CPU volume data stays direct. Scenario `gas_projection`; promoted to `ENGINE_LANDMINES.md`.
