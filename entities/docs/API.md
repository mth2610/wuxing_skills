...
* **Arena constants** (real-world-scaled, 1 unit = 1 meter — must match `maps/docs/API.md` §3 exactly — do not redefine elsewhere):
...
* Intended consumer: Buff-type skills (Core Agent's future "Entity-Attached" skeleton, `../../core/docs/API.md` §4 — not yet documented) write into this slot; this module only stores and ticks it down.
...
* While active: `combat/` grants clash immunity (combat/docs/API.md §2);
