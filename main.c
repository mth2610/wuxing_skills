...
int main(void) {
    ...
    while (!WindowShouldClose()) {
        ...
        if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT) && !uiState.clickedOnUI) {
            CastSkill(uiState.activeSkillIndex, player.position, mouseTarget3D, uiState.currentParams);
        }
        ...
        BeginDrawing();
            ...
            DrawText("Phím P: Đổi chế độ quái | Click trái: Tung chiêu | X: Khinh công", 10, 670, 20, LIGHTGRAY);
        EndDrawing();
    }
