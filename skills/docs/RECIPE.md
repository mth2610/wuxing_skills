...
See `../../core/docs/API.md` for full API details; this document focuses on **decisions and sequence**.
...
> correct but visually flat. `../../core/docs/COMPOSITION_API.md` §0 maps
...
ones) or `Update`/impact site (one-shot ones). Full param docs: `core/docs/API.md`
§19 "Group 3/4/5". Try them live first: `sandbox/vfx_test.c`'s **"NEW FX"** tab.
...
"pop" — see `core/docs/PROGRESS.md` Item 35. Use `VFX_ComposeGlintBurst`/`StreakFlare`
instead; they're LDR-safe and don't risk breaking on older GPUs.

**Element look & motion come from shared libraries, not hard-code** (core/docs/API.md
§19 Group 2b/2c): colors/gradients/force fields per element via
