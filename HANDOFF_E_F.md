...
**F0 (thanh lọc)** là việc của chủ project, đang làm. Bộ sống sót:
`VFX_ComposeSmokePuff`, `VFX_ComposeFlameVolume`, `VFX_ComposeEnergyBurst`,
`VFX_ComposeImpactPackage`.
...
| 2 | **E6 #7 `VFX_ComposeLightShaft`** | E1 đã có, không bị chặn |
| 3 | **E6 #8 `VFX_ComposeSpiritSwarm`** | không bị chặn |
| 4 | **E7 retrofit** | **chốt kiểm**: 3 skill mẫu, A/B với E0. Spec nói rõ đây là chỗ kế hoạch được chứng minh hoặc phải hoạch định lại |
| 5 | E8 platform/perf | |

**Hai mục OPEN đã ghi trong PROGRESS:**
...
- **Chủ project không đọc log trừ khi được nhờ.** Khi một câu hỏi chỉ log mới
  ...
  vòng debug bằng mắt.
- **Không tự chạy game được**: `./build/wuxing` chết ở `FATAL: RLVK: instance
  creation failed` ngoài phiên đồ hoạ của chủ project. Mọi câu hỏi runtime phải
...
./scripts/run_core_tests.sh      # 6/6 phải xanh trước khi báo xong
```
