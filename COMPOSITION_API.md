...
- `VFX_ComposeEnergySmoke(pos, scale, progress, time)` (`vc_smoke_energy.inl`, mới — thay thế hướng flipbook/video sau khi tìm/tạo asset atlas quá khó): **1 đốm khói duy nhất** toả ra chậm rồi tan biến. Vẽ trên **billboard quad camera-facing** (`DrawCoreBillboardQuad`, mới — `core/geometry/pm_core_shapes.inl`) thay vì sphere mesh — sphere luôn có silhouette hình tròn cứng về mặt hình học bất kể shading, quad phẳng thì hình dạng hoàn toàn do alpha shader quyết định. Rẻ hơn sphere nhiều (1 quad = 2 tam giác so với sphere 20×20 = 800 tam giác).
  > [!NOTE]
  ...
  > **Đổi shader lần 3 (cùng ngày) — nghiệm giải tích PT khuếch tán:** user đề xuất đúng hướng vật lý: vẽ 1 chấm trắng rồi khuếch tán theo PT Navier-Stokes/khuếch tán (∂C/∂t = D∇²C). Thay vì giải số bằng lưới (render-texture ping-pong + Jacobi/Gauss-Seidel mỗi frame — cần render target riêng cho mỗi puff, nặng hơn nhiều), dùng **nghiệm giải tích dạng đóng** cho nguồn điểm/đĩa 2D: `C(r,t) = C₀·exp(-r²/4Dt)/(4πDt)` — đúng là 1 hàm Gauss có phương sai tăng theo `t` (lan rộng) và biên độ đỉnh giảm theo `1/t` (mờ dần, tự động bảo toàn "khối lượng" — không cần tự chế fade curve riêng). `u_progress` map trực tiếp thành `t`, `u_diffusion` = hệ số `D`. Thêm FBM domain-warp lên toạ độ sample trước khi tính khoảng cách tâm.
  > **Sửa lần 4 (cùng ngày):** user báo "mờ ảo quá mà tròn lẳng" — vùng khuếch tán ban đầu quá lớn. Root cause: `t₀=0.05` (floor + mốc chuẩn hoá) đã cho half-width ~0.22 trong quad [-1,1] ngay lúc sinh — tức là 1 khối tròn mờ to sẵn từ frame đầu, không phải điểm nhỏ rồi lan dần. Giảm `t₀` xuống `0.005` + hiệu chỉnh lại `D` (composer: 0.35→0.18) sao cho half-width đi từ ~0.05 (điểm nhỏ) lúc sinh lên ~0.7 (phủ gần hết quad) lúc tan — đúng tỉ lệ "nhỏ → lan ra biên" yêu cầu. Bonus: cùng biên độ warp FBM giờ có tỉ trọng lớn hơn tương đối so với puff nhỏ hơn, tự nhiên phá vỡ bớt "tròn lẳng".
  > [!NOTE]
  > Bản đầu (4 sub-puff sphere ghép cụm + additive + threshold từ gần-0) bị chê "1 cục trắng xoá rồi tan biến" — sửa còn 1 sphere + alpha blend + threshold cao hơn + rim-fade, vẫn còn silhouette cứng do bản chất mesh sphere. User gợi ý tham khảo 1 shader raymarch explosion nổi tiếng (Shadertoy, Roman Komary) và đề xuất nên vẽ trên hình vuông/tròn thay vì mặt cầu — không dùng raymarch thật (quá đắt cho VFX phổ biến, đúng thang chi phí Black Hole đã né), nhưng áp dụng đúng insight cốt lõi: chuyển sang billboard quad để loại bỏ silhouette hình học cứng tận gốc, thay rim-fade bằng radial bias ngay trong density function.
