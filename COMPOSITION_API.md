...
- `VFX_ComposeEnergyFlow(from, to, scale, duration)` (`vc_archetype.inl`, mới 2026-07-10): kênh năng lượng mượt A→B với texture "chảy" cuộn liên tục, thay cho particle/jitter để bán cảm giác dòng chảy — dùng pool `Arch_Flow` (8 slot) giống hệt pattern `Arch_Bolt`, tự `EnergyFlow_Kill` khi hết `duration`. Khác `VFX_ComposeLightningBolt`: hình dạng KHÔNG re-jitter mỗi frame (1 đường cong nhẹ cố định qua `refHint` fixed lúc spawn), chỉ UV texture cuộn theo thời gian (`ProcRayConfig.flowScrollSpeed`) — nên track điểm đầu/cuối di chuyển mượt, không chớp giật như Bolt. Cơ chế nằm trong `core/vfx_proc_ray.c`'s `SpawnEnergyFlow`/`EnergyFlow_Update`/`EnergyFlow_Draw`/`EnergyFlow_Kill`, vẽ 2 pass qua `DrawRibbonStrip` (glow cộng màu không texture + core dùng `water_flow.png` cuộn UV theo arc-length).
- `VFX_ComposeGhostTendrils` — **đã xóa** (2026-07-10, cùng ngày với lúc thêm). Từng là N=5 strand `EnergyFlow` cycling hidden/visible độc lập (`Arch_TendrilGroup` pool trong `vc_archetype.inl`), dùng preset `ProcRay_GhostFlowConfig` (đã xóa theo). Bị chê "xấu hoắc" sau cả 2 lần chỉnh sửa (thickness, alpha) — xóa hẳn theo yêu cầu user thay vì tiếp tục vá. Nếu cần lại khái niệm "nhiều luồng năng lượng ngắt quãng từ nguồn đến mục tiêu" sau này, tham khảo cách tiếp cận cũ (multi-strand timing layer chồng lên `EnergyFlow`) làm điểm khởi đầu nhưng thiết kế lại phần shape/material — vấn đề gốc chưa rõ là do EnergyFlow's ribbon quá mảnh về bản chất, hay do cách phối màu/preset chưa đúng.
  ...
  >
  > **Migrate lần 2 cùng ngày:** `VFX_ComposeBeam` (`vc_beam.inl`) đã migrate sang ribbon module — 2 mặt phẳng giao nhau cũ (raw `rlBegin(RL_QUADS)`) thay bằng 2 lần gọi `DrawRibbonStripEx(..., RIBBON_FIXED_NORMAL, fixedNormal)`: pass 1 dùng `fixedNormal = perp2` (→ `side = tangent × perp2 = -perp1`, khớp mặt phẳng cũ theo perp1), pass 2 dùng `fixedNormal = perp1` (→ `side = perp2`, khớp mặt phẳng theo perp2) — vẫn giữ đúng kỹ thuật "2 mặt phẳng cố định theo `dir`, KHÔNG camera-facing" (khác mọi ribbon khác trong engine), chỉ là dựng qua module chung thay vì tay. UV scroll (`time * -2.0f`) giờ cộng vào `.v` sau `Ribbon_ComputeArcLengthUV` thay vì baked vào `rlTexCoord2f` thủ công. Jitter lightning giữ nguyên hành vi cũ (dịch cứng toàn bộ ribbon theo 1 giá trị `sinf(time*80)*0.05f`, không phải sóng riêng từng điểm). Phát hiện thêm 1 bug latent trong `DrawRibbonStripEx`: hàm không `rlSetTexture(0)` sau khi vẽ — vô hại với các caller cũ (luôn truyền `(Texture2D){0}`) nhưng rò texture state cho caller thật đầu tiên (`EnergyFlow`'s core pass, `VFX_ComposeBeam`) — đã fix thêm `rlSetTexture(0)` vào cuối `DrawRibbonStripEx` (`core/ribbon_strip.c`).
