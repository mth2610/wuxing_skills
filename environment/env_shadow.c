...
static void DumpAtTexel(const float *px, int res, const char *label, float u, float v) {
    ...
    for (int dy = -2; dy <= 2; dy++) {
        int yy = tyA + dy; if (yy < 0 || yy >= res) continue;
        char row[160]; int off = 0;
        for (int dx = -2; dx <= 2; dx++) {
            int xx = tx + dx; if (xx < 0 || xx >= res) { continue; }
...
void EnvShadow_DebugDump(Vector3 worldPos) {
    if (!s_ready) { TraceLog(LOG_INFO, "ENV_SHADOW dump: not ready"); return; }

    // Read back the R32F copy (the copy pass still runs every EndCapture).
    float *px = (float *)rlReadTexturePixels(s_copyRT.texture.id, s_resolution, s_resolution,
                                             RL_PIXELFORMAT_UNCOMPRESSED_R32);
    if (px == NULL) { TraceLog(LOG_WARNING, "ENV_SHADOW dump: rlReadTexturePixels returned NULL"); return; }
    ...
    for (int i = 0; i < total; i++) {
        ...
        if (d > mx) mx = d;
        if (d < 0.10f) bNear++;
        else if (d < 0.90f) bMid++;
        else if (d < 0.999f) bFar++;
        else bClear++;
    }
    ...
    // CPU says PASS but the screen shows blue/green there, FS z != CPU z.
    {
        ...
        for (int i = 0; i <= 12; i++) {
            ...
            if (tx < 0 || tx >= s_resolution || ty < 0 || ty >= s_resolution) continue;
