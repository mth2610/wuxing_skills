...
- **ENGINE** (`formation_system.h/.c`, written once): `formationPool[4]`
  (thiết kế §VI), mana-gated deploy (`Entity_TrySpendMana`), duration
...
typedef struct FormationDef {
    ...
    NatureZoneType resonantZone; // đè lên đúng zone → power 1.5 (NAT_NONE = never)
...
The remaining 3 designed formations (thiết kế) follow the same pattern.

## 5. Notes for def authors

- `Entity_AddModifier` (through `Entity_ApplyAoEBuff`) is refresh-not-stack
  for identical multipliers (ENTITIES_API.md §8) — re-applying your slow
  every tick is safe and is the intended idiom.
