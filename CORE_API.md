...
See **[SKILL_SKELETONS.md](SKILL_SKELETONS.md)** for all 4 skeleton templates:
...
See [SKILL_SKELETONS.md](SKILL_SKELETONS.md) end-of-file. Full API in §16 below.
...
##### GPU Instancing — standard pattern for "many copies of the same mesh, same frame"

Established while converting the ice-crystal burst from N `DrawMesh` calls to one `DrawMeshInstanced` (CORE_ISSUES.md Item 40). Use this pattern — don't improvise a new one — whenever a future shape (rock swarm, shard rain, blade storm, etc.) needs the same treatment.

**When to reach for this (and when not to):**
- Only after the simpler fixes are already in place and still insufficient: (1) build the mesh **once**, never per-cast (`ProceduralMesh_BuildXTemplateMesh` pattern above) — this alone kills the expensive part (`UploadMesh`/GPU-driver sync); (2) if that's done and you're still looping `DrawMesh` per instance, check whether that's actually measured as a bottleneck (CORE_ISSUES.md Item 39's audit-before-optimizing principle — rlgl/GPU budgets handle dozens of `DrawMesh` calls per frame without issue; don't reach for instancing just because a loop exists).
- Good fit: N ≳ 10+ copies of one static mesh per frame, where per-instance variation is fully expressible as a transform (position/rotation/scale) — e.g. a burst of identical crystals/rocks/shards scattered around a point.
- Bad fit / needs more work than this pattern covers: per-instance variation that ISN'T just a transform (e.g. each instance needs its own color, dissolve amount, or — as found here — its own grow-progress stagger). Instancing gives the whole batch **one shared set of fragment/vertex uniforms**; per-instance extra data needs a second custom instance-attribute buffer (real extra engineering, not covered by the reference implementation below — only build it if a concrete effect actually needs per-instance uniform variation, don't add it speculatively).

**Implementation checklist (mirror `crystal_instanced.vs` / `CrystalMaterialInstanced` / `VFX_DrawIceCrystalBurst`):**
...
5. **Duplicate the material wrapper, don't try to share one.** A separately-compiled shader program can have different uniform *locations* for the same-named uniform, so the `XMaterial`-style loc-cache struct can't be reused as-is — copy `CrystalMaterial`'s struct/`_Load`/`_Begin`/`_End` shape into a second `XMaterialInstanced` pointed at the new shader (see `CrystalMaterialInstanced` in `core/material/material_system.h/.c` for the reference copy). This is accepted, intentional duplication — not a place to build a shared-across-2-programs abstraction for what is, so far, a single use case.
