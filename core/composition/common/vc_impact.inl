...
void VFX_ComposeTriggerImpactBurst(Vector3 pos, float sizeScale, const ImpactBurstConfig *cfg)
{
    ...
    if (cfg->particlesEnabled)
    {
        ...
        Color startColor = p->gradient
                               ? ColorGradient_Sample(p->gradient, 0.0f)
                               : p->colorStart;
        Color endColor = p->gradient
                             ? ColorGradient_Sample(p->gradient, 1.0f)
                             : p->colorEnd;
...
void VFX_TriggerExplosion(VC_MaterialId matId, Vector3 pos, float scale, bool cameraShake)
{
    ...
    if (matId == VC_MAT_LIGHTNING)
        lightR = 4.0f;
    else if (matId == VC_MAT_HOLY)
        lightR = 4.5f;
    else if (matId == VC_MAT_VOID)
        lightR = 3.8f;
    else if (matId == VC_MAT_POISON)
        lightR = 3.2f;
    else if (matId == VC_MAT_ICE || matId == VC_MAT_EARTH)
        lightR = 3.0f;

    ImpactBurstConfig cfg = {
        .distortEnabled = true,
        .distortRadius = 2.0f * scale,
        .distortStrength = 0.3f,
        .distortLife = 0.4f,
        .distortSpeed = 4.5f,
