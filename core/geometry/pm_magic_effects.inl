...
void ProceduralMesh_DrawVortexFunnel(const VortexFunnelMeshData *data, Color color)
{
    ...
    {
        // Tính tọa độ V theo chiều dọc (0.0 ở đáy, 1.0 ở đỉnh)
        float v1 = (float)i / data->heightSegs;
        float v2 = (float)(i + 1) / data->heightSegs;

        for (int j = 0; j < data->radialSegs; j++)
        {
            int nextJ = (j + 1) % data->radialSegs;

            // Tính tọa độ U theo chu vi vòng tròn (0.0 -> 1.0)
            float u1 = (float)j / data->radialSegs;
            float u2 = (float)(j + 1) / data->radialSegs;

            // Vertex 1 (Dưới, Trái)
            rlNormal3f(data->normals[i][j].x, data->normals[i][j].y, data->normals[i][j].z);
            rlTexCoord2f(u1, v1); // <-- GẮN UV VÀO ĐÂY
            ...
            rlTexCoord2f(u2, v1); // <-- GẮN UV VÀO ĐÂY
            ...
            rlTexCoord2f(u2, v2); // <-- GẮN UV VÀO ĐÂY
            ...
            rlTexCoord2f(u1, v2); // <-- GẮN UV VÀO ĐÂY
...
void ProceduralMesh_BuildFissure(FissureMeshData *out, const Vector3 *pathPoints, int pathPointCount, float width, float depth, float jaggedness, int seed)
{
    ...
    for (int i = 0; i <= out->segments; i++)
    {
        ...
        out->verts[i][1] = Vector3Add(centerJ, Vector3Scale(across, -hw * 0.45f));
        out->verts[i][1].y += dShoulder + ProceduralMesh__Noise2(i, 4, seed) * depth * 0.2f * jaggedness;
        out->verts[i][2] = centerJ;
        out->verts[i][2].y += dBottom;
        out->verts[i][3] = Vector3Add(centerJ, Vector3Scale(across, hw * 0.45f));
        out->verts[i][3].y += dShoulder + ProceduralMesh__Noise2(i, 5, seed) * depth * 0.2f * jaggedness;
