...
void ProceduralMesh_DrawOrganicStonePillar(Vector3 pillarPos, float currentHeight, float baseRad, float topRad) {
    ...
    for (int h = 0; h < HEIGHT_SEGS; h++) {
        ...
        for (int r = 0; r < RADIAL_SEGS; r++) {
            ...
            rlNormal3f(normals[h][nextR].x, normals[h][nextR].y, normals[h][nextR].z); rlTexCoord2f(u2, v1); rlVertex3f(rings[h][nextR].x, rings[h][nextR].y, rings[h][nextR].z);
            rlNormal3f(normals[h][r].x, normals[h][r].y, normals[h][r].z); rlTexCoord2f(u1, v1); rlVertex3f(rings[h][r].x, rings[h][r].y, rings[h][r].z);
            rlNormal3f(normals[h + 1][r].x, normals[h + 1][r].y, normals[h + 1][r].z); rlTexCoord2f(u1, v2); rlVertex3f(rings[h + 1][r].x, rings[h + 1][r].y, rings[h + 1][r].z);
            rlNormal3f(normals[h + 1][nextR].x, normals[h + 1][nextR].y, normals[h + 1][nextR].z); rlTexCoord2f(u2, v2); rlVertex3f(rings[h + 1][nextR].x, rings[h + 1][nextR].y, rings[h + 1][nextR].z);
        }
    }
    rlEnd();
...
void ProceduralMesh_DrawOrganicPuddle(Vector3 pos, float radius) {
    ...
    float cosA[33]; float sinA[33]; float n[33];
    for (int i = 0; i <= sides; i++) {
        float angle = (i / (float)sides) * 2.0f * PI;
        cosA[i] = cosf(angle); sinA[i] = sinf(angle);
    ...
    for (int i = 0; i < sides; i++) {
        ...
        rlTexCoord2f(0.5f, 0.5f); rlVertex3f(pos.x, pos.y, pos.z);
        rlTexCoord2f(0.5f + 0.5f * n[i+1] * cosA[i+1], 0.5f + 0.5f * n[i+1] * sinA[i+1]); rlVertex3f(v2.x, v2.y, v2.z);
        rlTexCoord2f(0.5f + 0.5f * n[i] * cosA[i], 0.5f + 0.5f * n[i] * sinA[i]); rlVertex3f(v1.x, v1.y, v1.z);
    }
    rlEnd();
}