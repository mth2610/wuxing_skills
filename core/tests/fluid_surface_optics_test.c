...
// The causes are arithmetic, not rendering, so they are testable without a GPU:
//
//   1. The receiver depth gap was combined with max(), so an airborne body's
//      column was pinned at a constant 0.40 m — 2.5x the measured path. The
//      receiver may only BOUND the column (min), never create one.
//   2. The wave perturbation and the silhouette normal, below.
//
// The thickness DECODE this file used to mirror is gone: thickness is now
// measured as z_back - z_front and needs no decoding. Its guards moved to
...
int main(void)
{
    ...
    /* A measured path through an authored body: the dual-depth pass reports the
     * envelope of the splat cloud in metres. 0.20 m is what both sandbox
     * fixtures read through their middle (see FLUID_REFERENCE_DEPTH_M). */
    const float core = 0.20f;
