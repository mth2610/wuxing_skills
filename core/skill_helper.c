...
void SkillForceMix_AddLayers(const SkillForceMix *mix, ForceField *ff)
{
    if (mix->windStrength != 0.0f)
        ForceField_AddLayer(ff, (ForceLayer){
                                    .type = FORCE_WIND, .strength = mix->windStrength, .direction = {mix->windDirX, mix->windDirY, mix->windDirZ}, .noiseScale = mix->windNoiseScale, .noiseSpeed = mix->windNoiseSpeed});
    if (mix->perlinStrength != 0.0f)
        ForceField_AddLayer(ff, (ForceLayer){
                                    .type = FORCE_NOISE_PERLIN, .strength = mix->perlinStrength, .noiseScale = mix->perlinNoiseScale, .noiseSpeed = mix->perlinNoiseSpeed});
    if (mix->curlStrength != 0.0f)
        ForceField_AddLayer(ff, (ForceLayer){
                                    .type = FORCE_NOISE_CURL, .strength = mix->curlStrength, .noiseScale = mix->curlNoiseScale, .noiseSpeed = mix->curlNoiseSpeed});
    if (mix->gravDirStrength != 0.0f)
        ForceField_AddLayer(ff, (ForceLayer){
                                    .type = FORCE_GRAVITY_DIR, .strength = mix->gravDirStrength, .direction = {mix->gravDirX, mix->gravDirY, mix->gravDirZ}});
    if (mix->gravPtStrength != 0.0f)
        ForceField_AddLayer(ff, (ForceLayer){
                                    .type = FORCE_GRAVITY_POINT, .strength = mix->gravPtStrength, .origin = {mix->gravPtOriginX, mix->gravPtOriginY, mix->gravPtOriginZ}});
    if (mix->vortexStrength != 0.0f)
        ForceField_AddLayer(ff, (ForceLayer){
                                    .type = FORCE_VORTEX, .strength = mix->vortexStrength, .origin = {mix->vortexOriginX, mix->vortexOriginY, mix->vortexOriginZ}, .direction = {mix->vortexDirX, mix->vortexDirY, mix->vortexDirZ}});
...
{
    if (handle < 0 || handle >= SB_MAX_BEAMS || !s_beams[handle].active)
        return;
...
{
    if (count > SB_ORBITALS_PER_GROUP)
        count = SB_ORBITALS_PER_GROUP;
...
{
    if (handle < 0 || handle >= SB_MAX_AURAS || !s_auras[handle].active)
        return;
    for (int k = 0; k < SB_AURA_RING_K; k++)
...
void SkillBuilder_Update(float dt)
{
    ...
    for (int i = 0; i < SB_MAX_GROUNDWAVES; i++)
    {
        ...
        if (travelDist >= s_gwaves[i].range)
            s_gwaves[i].active = false;
    }
    for (int i = 0; i < SB_MAX_ORBITALS; i++)
    {
        if (!s_orbitals[i].active)
...
{
    if (!outPoints || maxOut <= 0 || maxJumps <= 0)
        return 0;
    ...
    while (found < maxJumps && found < maxOut)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            if (alreadyHit)
                continue;
            Vector3 p;
            if (!SkillManager_GetAgentPos(ids[i], &p))
                continue;
        ...
        Vector3 nextPos;
        if (!SkillManager_GetAgentPos(best, &nextPos))
            break;
        outPoints[found++] = nextPos;
        if (visitedCount < 16)
            visited[visitedCount++] = best;
        current = nextPos;
    }
    return found;
}
