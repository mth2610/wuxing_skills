...
`API.md` §5 and `../../compute/docs/API.md` §3. Added a `sandbox/vfx_test.c` "VF
...
Source: full audit of `core/` + `API.md` against the goal **"an AI
...
*What now exists (all documented in `API.md` — read there for full
...
before starting a new skill, not just `API.md` — the *pattern* (how
...
- *"`Entity_GetNearbyTargets` undocumented — only a comment mention"* — false.
  Full signature, semantics, and usage are documented in `../../entities/docs/API.md:112`
  §7 (and its AoE-composition callers in §8/§9). Correctly owned/documented by
  the Entities Agent, not a `API.md` gap — the reviewer only checked
  `API.md`.
- *"No terrain-height query API / flat-ground assumption undocumented"* —
  false. `maps/docs/API.md:294` explicitly documents the default arena as flat at
  `Y = 0.0f`, and `maps/docs/API.md:450` already ships a `GetHeightmapHeight()`
...
- *"No consolidated performance-budget table"* — minor point, not filed as an
  ...
  behavior inline (e.g. `API.md:778` trail pool, `:857` `MAX_DECALS`) —
...
- *"Self-acknowledged gaps (SFX assets, `Material_Load` broken preset paths,
  ...
  already are, inline: `API.md:1580` (SFX), `API.md:1756` (`Material_Load`
  paths, explicitly marked "known pre-existing issue, out of scope"), and
  `API.md:649`/`../../entities/docs/API.md` §9 (AoEBuff team filter). Nothing new to
...
convention (`maps/docs/API.md` §11).
...
- `core/flow_map.c`'s "bug cũ" (documented inline) and `PROGRESS.md` Item
...
in the codebase** — fog is effectively dead code today, `maps/docs/API.md` §4's
...
`API.md` under "GPU Instancing — standard pattern for 'many copies of
...
it; see the "bad fit" note in the API.md standard). All crystals in one
...
- **`VFX_ComposeFloatingStones`** (`core/composition/vc_earth.inl`) — was 5
  ...
  added to the API.md decision tree). This effect used `EffectMaterial`
...
decision guidance (Crystal-backed vs EffectMaterial-backed) in API.md's
...
`API.md`, `core/docs/EXTERNAL_API.md`. **`API.md` NOT updated** (still
