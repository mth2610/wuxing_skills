...
`agentId` is the caster's agent-pool slot (0..255), forwarded automatically by `CastSkill()` (PROGRESS.md Item 15). Store it in your per-instance struct as `int ownerAgentId;` at cast time — it's what lets `AbortSkill(skillIndex, agentId)` target only the caster's own instances. Ownership tracking is its only job right now; see `skills/CLAUDE.md` for the full rule.
...
```
* **`priority` (PROGRESS.md Item 12):** additive field — `TrailConfig cfg = {0};` still compiles and defaults to `VFX_PRIORITY_LOW`, so this is backward compatible (unlike `VFXLight_Spawn`'s signature change above). Set `cfg.priority = VFX_PRIORITY_HIGH_ULTIMATE;` for a cast that must not silently lose its trail to pool pressure.
...
See [core/docs/COMPOSITION_API.md](core/docs/COMPOSITION_API.md) — full `ImpactBurstConfig` struct, `VFX_ImpactPreset`, and `VFX_TriggerImpactBurst`.
...
PROGRESS.md Item 16. `Skill_CalculateCooldown()`/`Skill_CalculateManaCost()` only compute numbers — these two hold and check actual elapsed-time state:
```c
...
PROGRESS.md Item 14. Optional, additive — does **not** change the mandatory skill lifecycle contract in `skills/CLAUDE.md`:
```c
...
PROGRESS.md Item 13. Optional, additive — does **not** change the mandatory skill lifecycle contract in `skills/CLAUDE.md`:
```c
...
PROGRESS.md Item 17. Thin wireframe overlay for visually tuning hitbox/AoE radii — no equivalent existed before (`core/tuning.h`'s hot-reload let you edit a radius number but not see it in-world).
```c
...
See **[core/docs/SHADER_API.md](core/docs/SHADER_API.md)** for the full reference:
...
See **[core/docs/COMPOSITION_API.md](core/docs/COMPOSITION_API.md)** for the full composition reference:
...
> [!IMPORTANT]
> Both this file and `core/docs/COMPOSITION_API.md` are mechanical references (what
...
> `core/docs/COMPOSITION_API.md` §0's table to translate the cookbook pattern you pick
