...
- `VFX_DrawOpticalStarburstStreak(pos, coreCol, streakCol, starRadius, streakLength, streakThickness, intensity, camera)` (`vc_optical_flare.inl`): optical lens flare package with multi-ray diffraction starburst billboard and camera-right aligned anamorphic horizontal streak (no circular halo ring, centered on character mid-torso).
- `VFX_DrawModelSilhouetteGlow(model, transform, glowColor, intensity)` (`vc_silhouette_glow.inl`): high-contrast unlit incandescent silhouette pass with Fresnel edge bloom and surface-conforming energy motes on ANY Raylib `Model`.
- `VFX_DrawMeshSilhouetteGlow(mesh, transform, glowColor, intensity)` (`vc_silhouette_glow.inl`): draws silhouette glow on ANY arbitrary single `Mesh` with a given `Matrix` transform.
- `VFX_DrawCharacterSilhouetteGlow(playerPos, yaw, auraColor, intensity, targetMeshOrAnim)` (`vc_silhouette_glow.inl`): universal entry point — automatically renders the real character model (or custom `Model`/`Mesh`/`CharacterAnimState`) with zero fake geometry!
- `VFX_ComposeVacuumConverge(focalPoint, sphereRadius, progress, camera)` (`vc_vacuum_arc.inl`): 3D upper-hemisphere suction vortex streamlines. Atmospheric qi airflows spawn randomly across the upper hemisphere ($Y \ge 0$), spiraling inward with angular acceleration and converging into the focal point where they vanish.
- `VFX_DrawExpandingVacuumRing(center, radius, bandWidth, ringColor, alpha01)` (`vc_vacuum_ring.inl`): clean, razor-edged planar vacuum compression ring expanding smoothly on the ground with terrain height conforming.
- `VFX_ComposeIaidoStance(playerPos, yaw, progress, duration, camera, animStatePtr)` (`vc_iaido_stance.inl`): composite quick-draw / counter stance orchestrating all 4 primitives over `progress` (0.0 -> 1.0), with suction vortex converging into the sword hilt focal point. Triggerable in sandbox via key `[5]`.
- `VFX_ComposeGuidingWind(startPos, targetPos, progress, camera)` (`vc_guiding_wind.inl`): Ghost of Tsushima soft guiding wind ribbon cluster. Evaluates real-time streamline with `Wind_EvaluateVelocity` (Macro Wind + 256 Vorticles), look-ahead terrain height probing (`VFX_GroundHeightFromMap` + aerodynamic lift over slopes), custom dual-noise panned alpha erosion shader (`wind_ribbon.fs`), and 6 tumbling 3D Japanese maple leaves riding the air current.
