...
- `VFX_ComposeCentripetalSlash(basePos, yawAngle, matId, progress, duration, camera)` (`vc_sweep_slash.inl`): a full-bodied 3D Centripetal Catmull-Rom curved blade cleave. Uses dual ribbon geometry (camera-facing + planar blade disc), sinusoidal tapering to eliminate blunt edges, element color palette, high-speed tangent sparks (5.5cm-8cm), radial wind blast, and apex Qi flash.
- `VFX_ComposeCentripetalSlashEx(basePos, yawAngle, primaryColor, accentColor, progress, duration, camera)` (`vc_sweep_slash.inl`): custom-colored variant of the Centripetal Catmull-Rom blade cleave (e.g. white hot core + cyan frost rim).
- `VFX_EmitCharacterSkinAura(playerPos, yaw, colorStart, colorEnd, speed, count)` (`vc_character_aura.inl`): uniform O(1) barycentric surface sampling across the animated character mesh. Emits elemental aura particles flowing outward along surface normals without shell artifacts.
- `ProceduralMesh_DrawCrescentSlash(center, forward, right, radius, width, arcAngle, tiltAngle, color)` (`core/geometry/procedural_mesh_utils.h`): procedural 3D crescent ribbon mesh used in refraction passes (`ScreenDistort_BeginMeshPass`) for spatial distortion wave slashes.
