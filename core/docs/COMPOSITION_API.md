...
- `VFX_DrawOpticalStarburstStreak(pos, coreCol, streakCol, starRadius, streakLength, streakThickness, intensity, camera)` (`vc_optical_flare.inl`): optical lens flare package with multi-ray diffraction starburst billboard and camera-right aligned anamorphic horizontal streak.
...
- `VFX_DrawCurvedVacuumArc3D(p0, p1, p2, p3, rimColor, bodyColor, maxWidth, progress, duration, camera)` (`vc_vacuum_arc.inl`): sweeping 3D parametric spline ribbon with needle-sharp sinusoidal tapering, razor-sharp leading edge, and ethereal translucent vacuum smoke body.
...
- `VFX_ComposeIaidoStance(playerPos, yaw, progress, duration, camera, animStatePtr)` (`vc_iaido_stance.inl`): composite quick-draw / counter stance orchestrating all 4 primitives over `progress` (0.0 -> 1.0). Triggerable in sandbox via key `[5]`.
