...
- `VFX_MeshParticleEmitter_Spawn(desc)` (`vc_mesh_particle_emitter.inl`): pooled, deterministic surface sampling on arbitrary Raylib `Mesh` or `Model`; update its world transform each frame with `VFX_MeshParticleEmitter_SetTransform`.
- `VFX_DrawMeshSurfaceAura(mesh, transform, params)` / `VFX_DrawModelSurfaceAura(model, transform, params)` (`vc_mesh_surface_aura.inl`): restrained shader-only Fresnel overlay with independent material color, rim width, intensity, and opacity.
