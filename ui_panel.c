...
void UpdateUIPanel(Vector2 mousePos, UIPanelState *state) {
    ...
    // Xác định danh sách chiêu thức khả dụng (tất cả 7 chiêu)
...
void DrawUIPanel(const UIPanelState *state) {
    ...
    for (int i = 0; i < availableCount; i++) {
        ...
        Color btnColor = isSelected ? baseColor : (isHover ? ColorAlpha(baseColor, 0.6f) : ColorAlpha(baseColor, 0.3f));
        ...
        DrawText(btnText, (int)(skillButtons[i].x + (skillButtons[i].width - textWidth) / 2), (int)(skillButtons[i].y + 15), 12, WHITE);
    }
    ...
    for (int i = 0; i < 3; i++) {
        ...
        DrawText(sizeText, (int)rectSize[i].x + 25, (int)rectSize[i].y + 10, 15, WHITE);
    }
    ...
    for (int i = 0; i < 2; i++) {
        ...
        DrawText(anchorNames[i], (int)(rectAnchor[i].x + (rectAnchor[i].width - textW)/2), (int)rectAnchor[i].y + 12, 12, WHITE);
    }
    ...
    for (int i = 0; i < 3; i++) {
        ...
        DrawText(pathNames[i], (int)(rectPath[i].x + (rectPath[i].width - textW)/2), (int)rectPath[i].y + 12, 12, WHITE);
    }

    // Portals
    bool ptHover = CheckCollisionPointRec(mousePos, rectPortalToggle);
    Color ptCol = state->currentParams.showPortal ? DARKGREEN : (ptHover ? LIME : DARKGRAY);
    DrawRectangleRounded(rectPortalToggle, 0.2f, 10, ptCol);
    DrawRectangleRoundedLines(rectPortalToggle, 0.2f, 10, WHITE);
    DrawText(state->currentParams.showPortal ? "PORTALS: ON" : "PORTALS: OFF", (int)rectPortalToggle.x + 40, (int)rectPortalToggle.y + 10, 16, WHITE);
}
