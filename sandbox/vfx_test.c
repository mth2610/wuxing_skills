...
// 34 entries — edit scripts/vfx_test_manifest.json, then re-run sync_vfx_test.py
                                  ...
                                  Texture2D globalParticleTex)
{
    ...
    if (s_isPlayingMesh && (s_testCategory == TEST_CAT_MESH || s_testCategory == TEST_CAT_COMPOSER))
    {
        ...
        if (s_meshTime > 5.0f)
            s_isPlayingMesh = false; // Turn off mesh drawing after 5 seconds
    }
    ...
    if (s_isPanelOpen)
    {
        ...
        if (s_testCategory == TEST_CAT_IMPACT || s_testCategory == TEST_CAT_CAST || s_testCategory == TEST_CAT_PROJECTILE)
        {
            maxIdx = 8;
            names = s_elementNames;
        }
    ...
    if (clicked && !s_clickedOnUI)
    {
        ...
        else if (s_testCategory == TEST_CAT_COMPOSER)
        {
            ...
            if (s_testIndex == 0)
                VFX_ComposeSmokePuff(s_prefabStartPos, 1.0f);
            else if (s_testIndex == 1)
                VFX_ComposeSmokeTrail(s_prefabStartPos, composerEnd, 1.0f);
            ...
            else if (s_testIndex == 3)
            { // BOLT: từ tay nhân vật đến điểm click
...
void VFXTest_Draw3D(void)
{
    ...
    if (s_testFissure.active)
    {
        ...
        if (totalDist > 0.001f)
        {
            ...
            rlTexCoord2f(progress, 0.0f);
            rlVertex3f(p2.x, p2.y, p2.z);
            rlTexCoord2f(progress, 1.0f);
...
void VFXTest_SetRenderTarget(int newfxIndex, Vector3 spawnPos)
{
    ...
    // Fire oneshot effects immediately so particles exist before warmup frames.
    // Keep in sync with @gen:newfx_trigger block above when oneshots change.
    switch (newfxIndex)
    {
    case 5:
        VFX_ComposeShockwaveRing(spawnPos, 1.5f, 0.6f, (Color){255, 200, 80, 255});
        break;
    case 6:
        VFX_ComposeGlintBurst(spawnPos, 14, 0.4f, (Color){180, 230, 255, 255});
        break;
    case 7:
        VFX_ComposeEmberDrift(spawnPos, 0.8f, 12, (Color){255, 140, 60, 255});
        break;
    case 8:
        VFX_ComposeStreakFlare(spawnPos, 1.0f, (Color){255, 250, 220, 255});
        break;
    case 15:
        VFX_ComposeBloomBurst(spawnPos, 1.0f);
        break;
    case 18:
        VFX_ComposeShrapnelBurst(Vector3Add(spawnPos, (Vector3){0, 0.25f, 0}), 1.0f);
        break;
    case 19:
        VFX_ComposeRicochetSpark(Vector3Add(spawnPos, (Vector3){0, 0.5f, 0}),
                                 (Vector3){0.7f, 0.7f, 0.0f}, 1.0f);
        break;
    case 20:
        VFX_ComposeSplashBurst(spawnPos, 1.0f);
        break;
    case 23:
        VFX_ComposeGustSlash(Vector3Add(spawnPos, (Vector3){0, 0.3f, 0}),
                             (Vector3){1.0f, 0.0f, 0.0f}, 1.0f);
        break;
    case 27:
        VFX_ComposeRockBurst(spawnPos, 1.0f);
        break;
    default:
        break; // continuous — VFXTest_Draw3D drives via s_isPlayingMesh
    }
}