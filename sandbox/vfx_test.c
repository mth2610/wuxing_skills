...
static bool VFXTest_FireNewFx(int newfxIndex, Vector3 pos)
{
    ...
    case 0:
        ...
        return true;
    case 2:
        VFX_ComposeContactSpark(pos, VC_MAT_FIRE, 1.5f, 0.0f);
        return true;
    case 5:
        VFX_ComposeDebrisShards(pos, (Vector3){1.4f, 2.2f, 0.5f}, VC_MAT_METAL, 1.5f, 5);
        return true;
    case 6:
        VFX_ComposeDecal(pos, VC_MAT_FIRE, 1.5f, 0.0f, 1.5f);
        return true;
    case 8:
        if (s_vfxFixtureHandle[8] >= 0)
            VFX_KillEmberTrail(s_vfxFixtureHandle[8]);
        ...
        return true;
    case 9:
        VFX_ComposeEnergyBurst(pos, VC_MAT_FIRE, 1.5f, 1.0f);
        return true;
    case 13:
        VFX_ComposeImpactDust(pos, VC_MAT_EARTH, 1.5f, 0.0f);
        return true;
    case 14:
        VFX_ComposeImpactPackage(pos, (Vector3){0.0f, 1.0f, 0.0f}, VC_MAT_FIRE, 1.5f, 0.4f);
        return true;
    case 20:
        if (s_vfxFixtureHandle[20] >= 0)
            VFX_KillShieldShell(s_vfxFixtureHandle[20]);
        ...
        return true;
    case 23:
        VFX_ComposeSmokePuff(pos, VC_MAT_FIRE, 1.5f, 1.0f);
        return true;
    case 25:
        VFX_ComposeSparkTrail(pos, (Vector3){1.4f, 2.2f, 0.5f}, VC_MAT_FIRE, 1.0f, 2.0f);
        return true;
    case 33:
        VFX_ComposeFluidImpact(pos);
        return true;
    case 34:
        VFX_ComposeIceCrystal(pos, posSeed);
        return true;
    case 35:
        VFX_ComposeWaterOrb(Vector3Add(pos, (Vector3){-2.0f, 1.2f, 0.0f}), Vector3Add(pos, (Vector3){2.5f, 1.8f, 0.8f}));
        return true;
    default:
        return false;
    }
...
void VFXTest_Draw3D(void)
{
    ...
    if (s_isPlayingMesh)
    {
        ...
        else if (s_testCategory == TEST_CAT_NEWFX)
        {
            ...
            {
            case 1:
                VFX_ComposeChargeConverge(s_prefabStartPos, VC_MAT_FIRE, 1.5f, progress, 5);
                break;
            case 3:
                VFX_ComposeConvergeMotes(s_prefabStartPos, VC_MAT_FIRE, 1.5f, progress, 5);
                break;
            case 4:
                VFX_ComposeCoreGlow(s_prefabStartPos, VC_MAT_FIRE, 1.5f, progress);
                break;
            case 7:
                VFX_ComposeDissolveExit(s_prefabStartPos, VC_MAT_FIRE, 1.5f, progress);
                break;
            case 10:
                VFX_ComposeEnergyOrb(s_prefabStartPos, VC_MAT_FIRE, 1.5f, progress);
                break;
            case 11:
                VFX_ComposeGlintSparkle(s_prefabStartPos, VC_MAT_FIRE, 1.5f, s_meshTime);
                break;
            case 12:
                VFX_ComposeGroundWave(s_prefabStartPos, VC_MAT_EARTH, 1.5f, progress, VFX_GroundHeightFromMap, NULL);
                break;
            case 15:
                VFX_ComposeLightShaft(Vector3Add(s_prefabStartPos, (Vector3){-2.0f, 1.2f, 0.0f}), Vector3Add(s_prefabStartPos, (Vector3){2.5f, 1.8f, 0.8f}), VC_MAT_FIRE, 0.8f, 1.35f);
                break;
            case 16:
                VFX_ComposePortalDisc(s_prefabStartPos, (Vector3){0.0f, 1.0f, 0.0f}, VC_MAT_FIRE, 1.5f, progress);
                break;
            case 17:
            {
                ...
                if (s_meshTime < s_vfxFixtureLastTime[17] && s_vfxFixtureHandle[17] >= 0)
                    VFX_KillProjectile(s_vfxFixtureHandle[17]);
                if (s_meshTime < s_vfxFixtureLastTime[17])
                    s_vfxFixtureHandle[17] = -1;
            ...
            case 18:
            {
                ...
                if (s_meshTime < s_vfxFixtureLastTime[18] && s_vfxFixtureHandle[18] >= 0)
                    VFX_KillRibbonTrail(s_vfxFixtureHandle[18]);
                if (s_meshTime < s_vfxFixtureLastTime[18])
                    s_vfxFixtureHandle[18] = -1;
            ...
            case 19:
                VFX_ComposeRuneCircle(s_prefabStartPos, (Vector3){0.0f, 1.0f, 0.0f}, VC_MAT_FIRE, 1.5f, progress, 5);
                break;
            case 21:
                VFX_ComposeShockRing(s_prefabStartPos, (Vector3){0.0f, 1.0f, 0.0f}, VC_MAT_FIRE, 1.5f, progress);
                break;
            case 22:
            {
                if (s_meshTime < s_vfxFixtureLastTime[22] && s_vfxFixtureHandle[22] >= 0)
                    VFX_SmokeColumn_Stop(s_vfxFixtureHandle[22]);
                if (s_meshTime < s_vfxFixtureLastTime[22])
                    s_vfxFixtureHandle[22] = -1;
            ...
            case 24:
            {
                ...
                if (s_meshTime < s_vfxFixtureLastTime[24] && s_vfxFixtureHandle[24] >= 0)
                    VFX_SmokeTrail_Stop(s_vfxFixtureHandle[24]);
                if (s_meshTime < s_vfxFixtureLastTime[24])
                    s_vfxFixtureHandle[24] = -1;
                s_vfxFixtureLastTime[24] = s_meshTime;
                s_vfxFixtureXf[24] = MatrixTranslate(fixturePos.x, fixturePos.y, fixturePos.z);
                if (s_vfxFixtureHandle[24] < 0)
                    s_vfxFixtureHandle[24] = VFX_ComposeSmokeTrail(&s_vfxFixtureXf[24], VC_MAT_METAL, 0.0f, 1.0f, VFX_COLUMN_SMOKE, true);
                break;
            }
            case 26:
            {
                ...
                if (s_meshTime < s_vfxFixtureLastTime[26] && s_vfxFixtureHandle[26] >= 0)
                    KillTrail(s_vfxFixtureHandle[26]);
                if (s_meshTime < s_vfxFixtureLastTime[26])
                    s_vfxFixtureHandle[26] = -1;
            ...
            case 27:
                VFX_ComposeSweepSlash(s_prefabStartPos, (Vector3){1.0f, 0.0f, 0.0f}, VC_MAT_FIRE, 1.0f, 90.0f, progress);
                break;
            case 28:
            {
                float a = s_meshTime * 1.35f;
                ...
                if (s_meshTime < s_vfxFixtureLastTime[28] && s_vfxFixtureHandle[28] >= 0)
                    VFX_KillVolumeTrail(s_vfxFixtureHandle[28]);
                if (s_meshTime < s_vfxFixtureLastTime[28])
                    s_vfxFixtureHandle[28] = -1;
