...
static void VFXTest_StopFixtures(void)
{
    ...
    if (s_vfxFixtureHandle[37] >= 0)
        VFX_KillVolumeTrail(s_vfxFixtureHandle[37]);
...
static bool VFXTest_FireNewFx(int newfxIndex, Vector3 pos)
{
    ...
    case 16: VFX_ComposeImpactPackage(pos, (Vector3){0.0f, 1.0f, 0.0f}, VC_MAT_FIRE, 1.5f, 0.4f); return true;
    case 18: VFX_ComposeLightningArc(Vector3Add(pos, (Vector3){-2.0f, 1.2f, 0.0f}), Vector3Add(pos, (Vector3){2.5f, 1.8f, 0.8f}), VC_MAT_LIGHTNING, 0.055f); return true;
    case 19: VFX_ComposeLightningGroundRicochet(pos, VC_MAT_LIGHTNING, 1.0f, posSeed); return true;
    case 24:
        if (s_vfxFixtureHandle[24] >= 0) VFX_KillShieldShell(s_vfxFixtureHandle[24]);
        s_vfxFixtureHandle[24] = VFX_ShieldShell_Spawn(pos, VC_MAT_WATER, 1.5f, 1.0f);
        return true;
    case 27: VFX_ComposeSmokePuff(pos, VC_MAT_FIRE, 1.5f, 1.0f); return true;
    case 29: VFX_ComposeSparkTrail(pos, (Vector3){1.4f, 2.2f, 0.5f}, VC_MAT_FIRE, 1.0f, 2.0f); return true;
    case 42: VFX_ComposeFluidImpact(pos); return true;
    case 43: VFX_ComposeIceCrystal(pos, posSeed); return true;
    case 45: VFX_ComposeWaterOrb(Vector3Add(pos, (Vector3){-2.0f, 1.2f, 0.0f}), Vector3Add(pos, (Vector3){2.5f, 1.8f, 0.8f})); return true;
...
// 48 entries — auto-managed by sync_vfx_test.py
static const char* s_newFxNames[] = {
    ...
    "FLOW SHIELD", "GLINT SPARKLE", "GROUND WAVE", "IMPACT DUST", "IMPACT PACKAGE", "LIGHT SHAFT",
    "LIGHTNING ARC", "LIGHTNING IMPACT", "PORTAL DISC", "REF BANDS", "REF PARTICLES", "RUNE CIRCLE",
    "SHIELD SHELL", "SHOCK RING", "SMOKE COLUMN", "SMOKE PUFF", "SMOKE TRAIL", "SPARK TRAIL",
    "SWEEP SLASH", "TRAIL MAIN", "TRAIL ENERGY", "TRAIL BLADE", "TRAIL WISP", "TRAIL BACKDROP",
    "TRAIL SMOKE", "VOLUME TRAIL", "FISSURE STREAK", "STONE PILLAR", "FLAME VOLUME", "BLACK HOLE",
    ...
    if (s_isPlayingMesh)
    {
        ...
        else if (s_testCategory == TEST_CAT_NEWFX)
        {
          ...
          switch (s_testIndex) {
              ...
              case 13: VFX_ComposeGlintSparkle(s_prefabStartPos, VC_MAT_FIRE, 1.5f, s_meshTime); break;
              case 14: VFX_ComposeGroundWave(s_prefabStartPos, VC_MAT_EARTH, 1.5f, progress, VFX_GroundHeightFromMap, NULL); break;
              ...
              case 20: VFX_ComposePortalDisc(s_prefabStartPos, (Vector3){0.0f, 1.0f, 0.0f}, VC_MAT_FIRE, 1.5f, progress); break;
              case 21:
              {
                  ...
                  if (s_vfxFixtureHandle[21] < 0)
                      s_vfxFixtureHandle[21] = VFX_ComposeRefBands(s_prefabStartPos, 1.5f);
                  break;
              }
              case 22:
              {
                  ...
                  if (s_vfxFixtureHandle[22] < 0)
                      s_vfxFixtureHandle[22] = VFX_ComposeRefParticles(s_prefabStartPos, 1.5f);
                  break;
              }
              ...
              case 26:
              {
                  ...
                  if (s_vfxFixtureHandle[26] < 0)
                      s_vfxFixtureHandle[26] = VFX_ComposeSmokeColumn(s_prefabStartPos, VC_MAT_METAL, 0.55f, 5.0f, VFX_COLUMN_SMOKE, true);
                  break;
              }
              case 28:
              {
                  ...
                  if (s_vfxFixtureHandle[28] < 0)
                      s_vfxFixtureHandle[28] = VFX_ComposeSmokeTrail(&s_vfxFixtureXf[28], VC_MAT_METAL, 0.18f, 1.0f, VFX_COLUMN_SMOKE, true);
                  break;
              }
              case 30: VFX_ComposeSweepSlash(s_prefabStartPos, (Vector3){1.0f, 0.0f, 0.0f}, VC_MAT_FIRE, 1.0f, 90.0f, progress); break;
              case 31:
              {
                  float a = s_meshTime * 1.35f;
                  Vector3 fixturePos = Vector3Add(s_prefabStartPos,
                  ...
                  if (s_vfxFixtureHandle[31] < 0)
                      s_vfxFixtureHandle[31] = VFX_ComposeTrail(&s_vfxFixtureXf[31], VC_MAT_FIRE, 0.1f, 2.0f, TRAIL_PRESET_MAIN);
                  break;
              }
              case 32:
              {
                  ...
                  if (s_vfxFixtureHandle[32] < 0)
                      s_vfxFixtureHandle[32] = VFX_ComposeTrail(&s_vfxFixtureXf[32], VC_MAT_FIRE, 0.0f, 2.0f, TRAIL_PRESET_ENERGY);
                  break;
              }
              case 33:
              {
                  ...
                  if (s_vfxFixtureHandle[33] < 0)
                      s_vfxFixtureHandle[33] = VFX_ComposeTrail(&s_vfxFixtureXf[33], VC_MAT_FIRE, 0.1f, 2.0f, TRAIL_PRESET_BLADE);
                  break;
              }
              case 34:
              {
                  ...
                  if (s_vfxFixtureHandle[34] < 0)
                      s_vfxFixtureHandle[34] = VFX_ComposeTrail(&s_vfxFixtureXf[34], VC_MAT_FIRE, 0.1f, 2.0f, TRAIL_PRESET_WISP);
                  break;
              }
              case 35:
              {
                  ...
                  if (s_vfxFixtureHandle[35] < 0)
                      s_vfxFixtureHandle[35] = VFX_ComposeTrail(&s_vfxFixtureXf[35], VC_MAT_FIRE, 0.15f, 2.0f, TRAIL_PRESET_BACKDROP);
                  break;
              }
              case 36:
              {
                  ...
                  if (s_vfxFixtureHandle[36] < 0)
                      s_vfxFixtureHandle[36] = VFX_ComposeTrail(&s_vfxFixtureXf[36], VC_MAT_FIRE, 0.0f, 2.0f, TRAIL_PRESET_SMOKE);
                  break;
              }
              case 37:
              {
                  ...
                  if (s_vfxFixtureHandle[37] < 0)
                      s_vfxFixtureHandle[37] = VFX_ComposeVolumeTrail(&s_vfxFixtureXf[37], VC_MAT_FIRE, 1.5f, 2.0f, VOL_ENERGY);
                  break;
              }
              case 38: VFX_ComposeFissureStreak(Vector3Add(s_prefabStartPos, (Vector3){-2.0f, 1.2f, 0.0f}), Vector3Add(s_prefabStartPos, (Vector3){2.5f, 1.8f, 0.8f}), 0.1f, progress, s_meshTime); break;
              case 39: VFX_ComposeStonePillar(s_prefabStartPos, progress); break;
              case 40: VFX_ComposeFlameVolume(s_prefabStartPos, VC_MAT_FIRE, 1.5f, 1.0f); break;
              case 41: VFX_ComposeBlackHole(VC_MAT_FIRE, s_prefabStartPos, 1.5f, s_meshTime); break;
              case 44: VFX_ComposeLiquidBench(s_prefabStartPos, 1.1f, 1.0f); break;
              case 46: VFX_ComposeWaterRing(s_prefabStartPos, 0.9f, 1.0f); break;
              case 47: VFX_ComposeWaterStream(Vector3Add(s_prefabStartPos, (Vector3){-2.0f, 1.2f, 0.0f}), Vector3Add(Vector3Lerp(Vector3Add(s_prefabStartPos, (Vector3){-2.0f, 1.2f, 0.0f}), Vector3Add(s_prefabStartPos, (Vector3){2.5f, 1.8f, 0.8f}), 0.33f), (Vector3){0.0f, 0.9f, 0.7f}), Vector3Add(Vector3Lerp(Vector3Add(s_prefabStartPos, (Vector3){-2.0f, 1.2f, 0.0f}), Vector3Add(s_prefabStartPos, (Vector3){2.5f, 1.8f, 0.8f}), 0.66f), (Vector3){0.0f, 0.5f, -0.7f}), Vector3Add(s_prefabStartPos, (Vector3){2.5f, 1.8f, 0.8f}), 1.5f, progress, s_meshTime); break;
          }
// @gen:newfx_draw end
        }
    }
