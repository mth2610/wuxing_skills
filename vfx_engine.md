...
## Phase 4 — Skill Modules Còn Lại

Thứ tự triển khai theo độ phức tạp tăng dần:

| Thứ tự | Skill | Kỹ thuật đặc trưng cần có trước |
|---|---|---|
| 1 | **Hoả (Fire)** | `additive_soft.fs`, multi-stop gradient, sub-emitter (tàn lửa) |
| 2 | **Thuỷ (Water)** | Flow map shader, screen distortion, spline path |
| 3 | **Điện (Electric)** | `SplinePath` (tia sét phân nhánh), `rim_glow.fs`, camera shake |
| 4 | **Mộc (Wood)** | `SplinePath` (dây leo), decal (rễ cây trên đất), spritesheet (lá rơi) |

---

## Checklist nhanh — thứ tự build

```
[ ] 1.1  flow_map.fs
[ ] 1.2  sprite_anim.h
[ ] 1.3  color_gradient.h
[ ] 1.4  Shader library (4 shader cơ bản)
[ ] 2.1  camera_fx.h
[ ] 2.2  screen_distort.h
[ ] 2.3  post_fx.h (bloom trước, còn lại sau)
[ ] 3.1  Sub-emitter (ParticleConfig mở rộng)
[ ] 3.2  vfx_light.h
[ ] 3.3  decal_system.h
[ ] 3.4  spline_path.h
[ ] 4.x  Fire → Water → Electric → Wood skill
[ ] 5.x  Audio-visual sync
```