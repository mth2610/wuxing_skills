...
| `shock_ring_smoke.png` | `VFX_SurfaceRegistry` ShockRingSmoke body | **TILING polar strip** — thin swirling smoke filaments wrapped around the shock ring; seamless in x, zero at top/bottom so the smoke can be panned off the band. RGB neutral, A is true density. Simulated by `scripts/gen_shock_ring_smoke.py` (curl-noise advection, deterministic from `--seed`). | neutral RGBA / alpha coverage |
...
| `flipbooks/smoke_roil_normals_8x8.png` | SmokeRoilNiagara normal companion | 2048×2048, 8×8, 64 frames | preview-only; BC5-style XY |
| `flipbooks/smoke_puff_8x8.png` | SmokePuffDarkNiagara / `VFX_ComposeSmokeVolume` | 2048×2048, 8×8, 64 frames | preview-only |
| `flipbooks/smoke_puff_normals_8x8.png` | SmokePuffDarkNiagara normal companion | 2048×2048, 8×8, 64 frames | preview-only; BC5-style XY |
| `flipbooks/smoke_puff_light_8x8.png` | SmokePuffLightNiagara / `VFX_ComposeSmokeVolume` | 2048×2048, 8×8, 64 frames | preview-only |
| `flipbooks/smoke_puff_light_normals_8x8.png` | SmokePuffLightNiagara normal companion | 2048×2048, 8×8, 64 frames | preview-only; BC5-style XY |
...
| `flipbooks/smoke_wispy_normals_8x8.png` | SmokeWispyNiagara normal companion | 4096×4096, 8×8, 64 frames | preview-only; BC5-style XY; high-memory source |
...
## pure_flame_puff_8x8_volume.png (Đợt H — packed VOLUME sheet) — IN USE
...
python3 scripts/flipbook/pack.py build_cache/fire_volume_puff/frames --grid 8 --alpha-from-luma 0 --shape puff --out pure_flame_puff_8x8_volume.png --meta-out core/composition/fire/pure_flame_puff_metadata.inl --meta-symbol s_fvolVolumeFrameMeta
```
...
`pure_flame_puff_motion_8x8.png` is the matching `MOTION` atlas generated by
`scripts/flipbook/render_motion.py`. R/G store signed optical flow, B stores
normalised speed, and A masks valid moving content. It is sampled only for
subframe advection and counts as the profile's second texture.
